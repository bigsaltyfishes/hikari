...
use log::{debug, trace};
use crate::hal;
use crate::mem::address::{is_aligned, PhysicalAddress, VirtualAddress};
use crate::mem::defs::MMUFlags;
use crate::hal::memory::utils::MemoryHAL;
use crate::mem::frame::PhysicalFrame;
use crate::mem::paging::{Page, PageSize, PagingError, PagingResult};
use crate::hal::memory::vm::VmHAL;

// Re-export the HAL implementation.
pub mod implement {
    pub use crate::arch::hal_impl::memory::table::{PageTable, PageTableEntry};
}
...
pub trait GenericPageTable: Sync + Send {
    ...
    fn map_range(&mut self, start_virt: VirtualAddress, start_phys: PhysicalAddress, size: usize, flags: MMUFlags) -> PagingResult {
        assert!(is_aligned(start_virt));
        assert!(is_aligned(start_phys));
...
impl<const LEVEL: usize, PTE: GenericPTE> PageTableImpl<LEVEL, PTE> {
    ...
    fn get_entry_mut_inner(&mut self, virt: VirtualAddress, target_page_size: Option<PageSize>, auto_grow: bool) -> PagingResult<(&mut PTE, PageSize)> {
        let mut phys_to_mut = |phys: PhysicalAddress| {
            let ptr = hal::memory::utils::implement::phys_to_virt(phys) as *mut PTE;
    ...
    pub(crate) unsafe fn activate(&self) {
        hal::memory::vm::implement::activate(self.root.phys_addr());
    }
}
...
impl<const LEVEL: usize, PTE: GenericPTE> PageTableImpl<LEVEL, PTE> {
    ...
    pub fn clone_kernel_space(&self) -> Self {
        let pt = Self::new();
        hal::memory::vm::implement::map_kernel_space(pt.table_phys(), self.table_phys());
        pt
    }

    pub fn from_active() -> Self {
        let root = unsafe { hal::memory::vm::implement::current_addr() };
...
impl<const LEVEL: usize, PTE: GenericPTE> GenericPageTable for PageTableImpl<LEVEL, PTE> {
    ...
    fn map(&mut self, page: Page, phys: PhysicalAddress, flags: MMUFlags) -> PagingResult {
        ...
        hal::memory::vm::implement::flush_tlb(Some(page.virt_addr));
    ...
    fn unmap(&mut self, virt: VirtualAddress) -> PagingResult<(PhysicalAddress, PageSize)> {
        ...
        let phys = entry.addr();
        entry.clear();
        hal::memory::vm::implement::flush_tlb(Some(virt));
